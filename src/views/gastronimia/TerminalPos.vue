<template>
  <div class="h-[100dvh] w-full bg-[#0a0a0a] text-white font-sans flex flex-col md:flex-row overflow-hidden relative antialiased select-none">
    ...
    <main class="flex-1 flex flex-col h-[100dvh] border-r border-white/10 relative z-10">
      ...
      <header class="bg-[#111] p-4 md:p-6 flex justify-between items-center border-b border-white/10 relative z-20 shadow-md">
        <div>
      ...
      <section class="p-4 md:p-5 bg-black border-b border-white/5 flex flex-col gap-4 relative z-10 shadow-sm" aria-label="Filtros del menú">
        <div class="relative group">
          <input
            ...
            class="w-full bg-white/5 border border-white/10 rounded-full py-3 pl-12 pr-4 text-sm outline-none focus:border-[#D4AF37]/50 focus:bg-white/10 focus:ring-4 focus:ring-[#D4AF37]/10 transition-all duration-300 text-white placeholder:text-neutral-500 font-light"
          />
          <span class="absolute left-5 top-3.5 text-neutral-500 group-focus-within:text-[#D4AF37] transition-colors" aria-hidden="true">🔍</span>
        </div>
            ...
            >
              ...
              <div class="mt-4 flex justify-end">
                <button 
                  ...
                  class="w-10 h-10 md:w-12 md:h-12 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/30 text-[#D4AF37] flex items-center justify-center font-bold text-xl md:text-2xl hover:bg-[#D4AF37] hover:text-black transition-colors duration-300 shadow-inner active:scale-90 focus:outline-none focus:ring-2 focus:ring-[#D4AF37]"
    ...
    <div v-if="panelMesasAbierto" @click="panelMesasAbierto = false" class="fixed inset-0 bg-black/80 z-[100] backdrop-blur-sm transition-opacity" aria-hidden="true"></div>
    
    <aside 
      :class="[
        'fixed top-0 right-0 bottom-0 w-full md:w-[400px] bg-[#111] border-l border-white/10 z-[110] transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col shadow-[-20px_0_50px_rgba(0,0,0,0.8)]', 
    ...
    >
      <header class="p-5 md:p-6 border-b border-white/10 flex justify-between items-center bg-black">
        ...
        <button @click="panelMesasAbierto = false" class="text-neutral-500 hover:text-white hover:bg-white/10 p-2 bg-white/5 rounded-full transition-colors active:scale-95 focus:outline-none">
      ...
      <div class="flex-1 overflow-y-auto p-5 md:p-6 space-y-4">
        ...
        <article 
          ...
          class="bg-[#050505] border border-[#D4AF37]/30 p-5 rounded-2xl cursor-pointer hover:border-[#D4AF37] hover:shadow-[0_5px_20px_rgba(212,175,55,0.1)] transition-all duration-300 flex flex-col justify-between group active:scale-[0.98]"
        >
          ...
          <div class="mt-3 text-center text-[10px] text-[#D4AF37]/60 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity font-bold">
    ...
    >
      ...
      <div class="p-5 md:p-6 border-b border-white/10 bg-black relative">
        ...
        <input 
          ...
          class="w-full bg-white/5 border border-[#D4AF37]/30 p-4 md:p-5 rounded-xl focus:border-[#D4AF37] focus:ring-2 focus:ring-[#D4AF37]/20 text-white outline-none mt-3 text-center text-xl md:text-2xl font-bold font-serif disabled:opacity-50 disabled:bg-black transition-all duration-300 placeholder:text-neutral-600" 
        />
      </div>

      <div class="flex-1 overflow-y-auto p-4 md:p-5 space-y-4 relative bg-[#0a0a0a]">
        <transition name="fade">
          <div v-if="cargandoEdicion" class="absolute inset-0 bg-black/70 backdrop-blur-md z-10 flex items-center justify-center">
            <span class="text-[#D4AF37] animate-pulse text-xs uppercase tracking-widest font-bold">Cargando pedido...</span>
          </div>
        </transition>

        <div v-if="carrito.length === 0" class="text-center text-neutral-500 text-sm uppercase tracking-widest mt-12 px-4 font-bold border border-dashed border-white/10 p-6 rounded-2xl">
        ...
        <transition-group name="list" tag="div" class="space-y-4">
          <article v-for="(item, index) in carrito" :key="item.id_unico" class="bg-[#111] border border-white/5 p-4 md:p-5 rounded-2xl shadow-sm">
            ...
            <input 
              ...
              class="w-full bg-white/5 border border-white/10 p-3 rounded-xl text-xs md:text-sm text-white focus:border-[#D4AF37]/50 focus:bg-white/10 outline-none placeholder:text-neutral-600 italic transition-colors" 
            />
          </article>
        </transition-group>
      </div>
      ...
      <footer class="p-5 md:p-6 bg-[#050505] border-t border-white/10 shadow-[0_-15px_30px_rgba(0,0,0,0.6)] z-10 relative">
        ...
        <div class="flex gap-3 md:gap-4">
          <button 
            v-if="!ordenActualId" 
            @click="cancelarOrden" 
            class="w-1/3 py-4 md:py-5 rounded-xl border border-red-500/30 text-red-500 text-xs md:text-sm uppercase font-bold tracking-widest hover:bg-red-500/10 active:scale-95 transition-all focus:outline-none focus:ring-2 focus:ring-red-500/50"
          >
            Cancelar
          </button>
          <button 
            v-else 
            @click="volverAtras" 
            class="w-1/3 py-4 md:py-5 rounded-xl border border-neutral-500/30 text-neutral-400 text-xs md:text-sm uppercase font-bold tracking-widest hover:bg-neutral-500/10 hover:text-white active:scale-95 transition-all focus:outline-none focus:ring-2 focus:ring-white/50"
          >
            Volver
          </button>
          ...
          <button 
            ...
            :class="[
              'flex-1 py-4 md:py-5 rounded-xl text-black text-xs md:text-sm uppercase font-bold tracking-widest transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-yellow-500', 
...
// =====================================
// RENDIMIENTO Y ESTRUCTURA: Constantes extraídas del ciclo de vida reactivo
// =====================================
...
// =====================================
// 🛡️ SEGURIDAD ESTRICTA Y ANTI-SEO
// =====================================
const blindarTerminal = async () => {
  // 1. Ocultamiento total de rastreadores web (Anti-SEO)
  ...
  // 2. Control riguroso de sesión y roles (Lógica intacta reforzada)
  ...
  if (error || !perfil || (perfil.rol !== 'mesera' && perfil.rol !== 'mesera_temporal')) {
    // Si un usuario normal (o incluso un admin que no debe usar POS) entra, es expulsado inmediatamente.
    await supabase.auth.signOut()
    router.push('/login-pos')
    return
  }

  // Inicialización segura de credenciales operativas
...
// =====================================
// LÓGICA DE NEGOCIO SUPABASE (Intacta)
// =====================================
...
const menuFiltradoYBusqueda = computed(() => {
  ...
  if (busquedaMenu.value.trim() !== '') {
    // Normalización de cadenas optimizada
...
// =====================================
// ACCIONES DEL CARRITO Y ENVÍO (Lógica intacta)
// =====================================
