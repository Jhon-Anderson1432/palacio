<template>
  <!-- SEO (Anti-SEO) y Accesibilidad: Se usa role="application" para indicar que no es una página web de contenido -->
  <main 
  ...
  >
    ...
    <!-- HEADER -->
    <header class="bg-[#111] p-5 border-b border-[#D4AF37]/20 flex justify-between items-center shadow-[0_4px_20px_rgba(0,0,0,0.6)] z-20 relative">
      <div>
    ...
    <section class="flex-1 relative bg-black flex flex-col items-center justify-center z-10" aria-label="Cámara de escaneo QR">
      ...
      <div class="w-full h-full max-w-md mx-auto relative shadow-[0_0_50px_rgba(212,175,55,0.05)] bg-black overflow-hidden">
        ...
        <transition name="fade">
          <div v-if="cargandoCamara" class="absolute inset-0 flex items-center justify-center bg-black/90 backdrop-blur-sm z-30">
        ...
        <!-- Componente de Cámara (Rendimiento: Renderizado continuo sin v-if para evitar reinicios lentos de hardware) -->
        <qrcode-stream 
          @detect="onDetect" 
        ...
        <div class="absolute inset-0 pointer-events-none border-[40px] border-black/70 flex items-center justify-center z-20 transition-all duration-500">
          <div class="w-64 h-64 border-2 border-[#D4AF37]/70 rounded-3xl relative shadow-[0_0_20px_rgba(212,175,55,0.2)]">
            <!-- Animación de escaneo (Láser sutil) -->
            <div class="absolute top-0 left-0 w-full h-[2px] bg-[#D4AF37] shadow-[0_0_8px_#D4AF37] opacity-60 animate-scan"></div>
    ...
    <transition name="modal-slide">
      <div 
        ...
        class="absolute inset-0 z-50 bg-black/90 backdrop-blur-md flex flex-col p-6 items-center justify-center"
        role="dialog"
        aria-modal="true"
      >
        <div class="w-full max-w-md bg-[#0a0a0a] border border-[#D4AF37]/20 rounded-3xl p-6 shadow-[0_20px_50px_rgba(0,0,0,0.8)] flex flex-col transform transition-all">
          
          <div class="text-center mb-6">
            <div class="w-12 h-12 rounded-full bg-[#D4AF37]/10 flex items-center justify-center mx-auto mb-3">
          ...
          <form @submit.prevent="guardarRegistro" class="w-full space-y-4">
            ...
            <div class="space-y-1">
              <label for="nombreVigilante" class="text-[10px] text-neutral-400 uppercase tracking-widest font-bold ml-1">
              ...
              <input 
                ...
                class="w-full bg-black/50 border border-white/10 rounded-xl p-4 text-white text-sm outline-none focus:border-[#D4AF37]/60 focus:bg-white/5 transition-all duration-300 placeholder:text-neutral-600 font-light"
              />
            </div>

            <div class="space-y-1">
              <label for="observacion" class="text-[10px] text-neutral-400 uppercase tracking-widest font-bold ml-1">
              ...
              <textarea 
                ...
                class="w-full bg-black/50 border border-white/10 rounded-xl p-4 text-white text-sm outline-none focus:border-[#D4AF37]/60 focus:bg-white/5 transition-all duration-300 placeholder:text-neutral-600 font-light resize-none"
              ></textarea>
            </div>
            ...
            <div class="pt-4 flex gap-3">
              <button 
                ...
                class="w-1/3 border border-red-500/30 text-red-500 font-bold uppercase tracking-widest text-xs py-4 rounded-xl hover:bg-red-500/10 active:scale-95 transition-all duration-300"
              >
                Cancelar
              </button>
              <button 
                type="submit" 
                :disabled="guardando"
                class="flex-1 bg-[#D4AF37] text-black font-bold uppercase tracking-widest text-xs py-4 rounded-xl hover:bg-yellow-500 hover:shadow-[0_0_20px_rgba(212,175,55,0.4)] active:scale-95 transition-all duration-300 disabled:opacity-50 disabled:pointer-events-none"
              >
...
// ==========================================
// 🛡️ PILLAR SEO: Anti-SEO (Capa de Invisibilidad)
// ==========================================
onMounted(() => {
  // Aseguramos de que ningún robot de indexación rastree el módulo de vigilancia
...
const onDetect = async (detectedCodes) => {
  ...
  try {
    ...
    if (!puntoData) {
      alert("❌ Código QR no encontrado en la base de datos de seguridad.")
...
const guardarRegistro = async () => {
  ...
  try {
    ...
    // UX: Limpieza y reseteo visual sin alert intrusivo para flujo de trabajo rápido
    ...
    // Pequeño delay de enfriamiento para evitar escaneos dobles del mismo código
...
// RENDIMIENTO: Optimización del trazado en Canvas, asignando estilos solo si es necesario
...
<style scoped>
/* RENDIMIENTO: Suavizado por hardware */
.antialiased {
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
}

/* Transiciones modales nativas optimizadas por GPU */
...
/* UI/UX Premium: Animación de "Láser" de escaneo */
.animate-scan {
...
/* Eliminar borde azul por defecto en inputs móviles */
