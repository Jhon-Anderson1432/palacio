<template>
  <!-- ESTRUCTURA: Uso de <main> semántico y selección de colores blindada -->
  <main class="min-h-screen bg-[#0a0a0a] text-white font-sans flex items-center justify-center p-4 selection:bg-[#D4AF37] selection:text-black relative overflow-hidden">
    ...
    <!-- ESTILOS: Fondo ambiental optimizado -->
    ...
    <div class="relative z-10 w-full max-w-md bg-[#111] border border-white/10 p-8 md:p-10 rounded-[2rem] shadow-2xl backdrop-blur-xl animate-fade-in-up">
      ...
      <!-- RENDIMIENTO & SEGURIDAD: autocomplete="off" y spellcheck="false" previenen filtraciones -->
      <form @submit.prevent="handleLogin" class="space-y-6" autocomplete="off" novalidate>
        ...
        <div class="space-y-2 group">
          ...
          <input 
            ...
            class="w-full bg-black/50 border border-white/10 text-white p-4 rounded-xl focus:border-[#D4AF37] focus:bg-black focus:ring-1 focus:ring-[#D4AF37]/50 outline-none transition-all duration-300 text-sm placeholder:text-neutral-700" 
          />
        </div>
        ...
        <button 
          type="submit" 
          :disabled="cargando" 
          class="w-full py-4 mt-4 bg-[#D4AF37] text-black font-bold uppercase tracking-[0.2em] rounded-xl hover:bg-white hover:shadow-[0_0_20px_rgba(212,175,55,0.4)] active:scale-[0.98] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-xs relative overflow-hidden"
        >
...
// === ANTI-SEO (EL BLINDAJE PARA EL NAVEGADOR) ===
const inyectarAntiSEO = () => {
  // 1. Título genérico (No usar palabras clave como "Palacio Nacional", "Admin", etc.)
  document.title = "Acceso Restringido";

  // 2. Directivas para Motores de Búsqueda: No indexar, no seguir enlaces, no guardar caché, no mostrar fragmentos
  const metaTags = [
    { name: 'robots', content: 'noindex, nofollow, noarchive, nosnippet' },
    { name: 'googlebot', content: 'noindex, nofollow' },
    // 3. Directivas de Caché HTTP para evitar que el navegador guarde la página
...
// LÓGICA DE NEGOCIO (Totalmente Intacta y Segura)
...
// Ejecutar el blindaje apenas cargue el componente
onMounted(() => {
  inyectarAntiSEO();
})
</script>
...
<style scoped>
/* ESTILOS: Animaciones suaves de entrada y transiciones de error */
